...
lint-python:
    ...
    if command -v ruff >/dev/null 2>&1; then
        find . -name "*.py" -not -path "./.claude/*" | xargs -r ruff check
    else
        echo "ruff not found - install with: brew install ruff"
    fi

# Lint JavaScript/TypeScript files with biome
lint-js:
    #!/usr/bin/env bash
    echo "Linting JavaScript/TypeScript files..."
    if command -v biome >/dev/null 2>&1; then
        find . -name "*.js" -o -name "*.ts" -o -name "*.jsx" -o -name "*.tsx" | \
        grep -v node_modules | grep -v .claude | xargs -r biome lint
    else
...
lint-shell:
    ...
    if command -v shellcheck >/dev/null 2>&1; then
        ...
        grep -v .claude | xargs -r shellcheck -x -e SC1090,SC1091
...
lint-markdown:
    ...
    if command -v markdownlint >/dev/null 2>&1; then
        find . -name "*.md" -not -path "./.claude/*" | xargs -r markdownlint
    else
